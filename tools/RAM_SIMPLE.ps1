# RAM_SIMPLE.ps1 - Simplified Interactive Mode
...
$RAMConfig = @{
    ...
    CoreScript = "D:\01 Main Work\Boots\Agentic AI\mission-control\tools\RAM\core\RAM-core-ai-integrated.ps1"
...
function Parse-SimpleIntent {
    ...
    # Strip "RAM>" prefix if user accidentally types it
...
function Execute-SafeIntent {
    ...
    switch ($Intent.action) {
        "help" {
            ...
            Write-Host "║          RAM LOCAL - COMMANDS           ║" -ForegroundColor Cyan
            Write-Host "╚════════════════════════════════════════════╝" -ForegroundColor Cyan
            Write-Host ""
        ...
        "status" {
            ...
            Write-Host "╚════════════════════════════════════════════╝" -ForegroundColor Cyan
            Write-Host ""
            Write-Host "Name:       RAM LOCAL"
            Write-Host "Version:    0.3-simple"
            Write-Host "Provider:   $($RAMConfig.Provider)"
...
function Start-InteractiveLoop {
    Write-Host "╔════════════════════════════════════════════╗" -ForegroundColor Cyan
    Write-Host "║     RAM - ราม LOCAL v0.3-oracle             ║" -ForegroundColor Cyan
    Write-Host "║     Simple Interactive Mode              ║" -ForegroundColor Cyan
    Write-Host "╚════════════════════════════════════════════╝" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "Provider:   $($RAMConfig.Provider)" -ForegroundColor Gray
